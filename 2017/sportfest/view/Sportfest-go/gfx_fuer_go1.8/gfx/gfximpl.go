...
// Datum: 07.03.2016 ; letzte Änderung: 12.03.2017
// Zweck: - Grafik- und Soundausgabe und Eingabe per Tastatur und Maus
//          mit Go unter Windows und unter Linux
//        - Eine Anpassung an Go 1.5+ war notwendig, da der "Unterbau"
//          von Go verändert wurde 

/*
#cgo LDFLAGS: -lSDL -lSDL_gfx -lSDL_ttf -lX11
#include <SDL/SDL.h>
...
//------------------------------------------------------------------
static void setFont (char *fontfile, int groesse) {
...
func SetzeFont (s string, groesse int) {
	lock ()
	cs:=C.CString(s)
	defer C.free(unsafe.Pointer(cs))
	C.setFont(cs,C.int(groesse))
	unlock()
}
